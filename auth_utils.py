...
# --- STRIPE PAYMENTS ---
def create_stripe_checkout(user_email, price_id):
    """
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            line_items=[{
                'price': price_id, # The ID from your Stripe Dashboard (price_123...)
                'quantity': 1,
            }],
            mode='subscription',
...
def verify_stripe_payment(session_id):
    """
    Checks with Stripe if the session was paid and returns the plan name.
    """
    if "stripe" not in st.secrets: return None
    stripe.api_key = st.secrets["stripe"]["secret_key"]
    ...
    try:
        ...
        if session.payment_status == 'paid':
            # Retrieve the product to know which tier they bought
            # Note: For simplicity, we will map Price IDs to Tier Names in the main app logic
            # or just return the Price ID here.
            
            # We return the Price ID so the app can decide which tier it corresponds to
...
def create_stripe_portal_session(user_email):
    ...
    try:
        ...
        # (Since we didn't store customer_id in Supabase, we look it up by email)
