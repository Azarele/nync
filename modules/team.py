...
import time
import pytz
from modules import scheduler # <-- Import the scheduler
...
def show(user, supabase):
    ...
    # ==========================================
    # --- NEW: TEAM TABS (Roster vs Heatmap) ---
    # ==========================================
    tab_settings, tab_heatmap = st.tabs(["👥 Roster & Settings", "🗺️ Availability Heatmap"])
    ...
    with tab_heatmap:
        # Fetch the roster specifically for the heatmap
        roster = auth.get_team_roster(selected_tid)
        scheduler.show(supabase, user, roster)